...
RUN --mount=type=cache,id=s/8f281236-4a82-4f29-85d2-c8ab2959997a-apt-cache,target=/var/cache/apt,sharing=locked \
  --mount=type=cache,id=s/8f281236-4a82-4f29-85d2-c8ab2959997a-apt-lists,target=/var/lib/apt/lists,sharing=locked \
  apt-get update \
  && apt-get upgrade -y \
...
RUN --mount=type=cache,id=s/8f281236-4a82-4f29-85d2-c8ab2959997a-npm-cache,target=/root/.npm \
...
COPY . ./
RUN --mount=type=cache,id=s/8f281236-4a82-4f29-85d2-c8ab2959997a-next-cache,target=/app/.build/next/cache \
  mkdir -p /app/data \
  && npm run build \
